...
@app.route('/satisfy/<dep>,<des>') # URL뒤에 <>을 이용해 가변 경로를 적는다
def location(dep,des):
    ...
    temp.reverse()
    return '%f %f'%(temp[0][0],temp[0][1])

@app.route('/<dep>,<des>') # URL뒤에 <>을 이용해 가변 경로를 적는다
def location2(dep,des):
    dep_lat, dep_lng = getlatlng(dep)
    des_lat, des_lng = getlatlng(des)
    s = getPos(dep_lat, dep_lng)
    e = getPos(des_lat, des_lng)
